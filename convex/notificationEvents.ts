/**
 * CATALOGUE des événements notifiables hors-app — RÉPLIQUE SERVEUR de
 * `lib/notification-events.ts` (règle A6 : un module `convex/` ne peut pas
 * importer `lib/`). DOIT rester STRICTEMENT IDENTIQUE (mêmes clés, mêmes
 * libellés, même classement). La parité est verrouillée par
 * `lib/notification-events.test.ts`, qui importe LES DEUX versions.
 *
 * Module PUR (aucun import `_generated`) : importé par `convex/notifications.ts`
 * pour savoir si un événement est activé sur le projet.
 */
